...
<template>
  <v-table fixed-header height="900">
    <thead>
      <tr>
    ...
    <tbody>
      <tr v-for="(item, index) in inspections" :key="index">
        ...
        >
          <v-btn color="error" disabled>STOP</v-btn>
        </td>
        ...
        >
          <v-btn color="primary" disabled>SIGN</v-btn>
        </td>
        <td
          class="text-center"
        ...
        <td class="text-center bg-success" v-if="item.managerJudgement == 1">
          OK
        </td>
        <td class="text-center bg-error" v-if="item.managerJudgement == 2">
          STOP
        </td>
        <!-- managerCheck end -->
      </tr>
