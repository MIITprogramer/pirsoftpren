      ...
      <v-table density="compact">
        ...
        <tbody>
          ...
          <tr v-for="(item, index) in part.inspectionItem" :key="item">
            ...
            <td>{{ item.itemName }}</td>
            <td>{{ method.find((e) => e.id == item.method).methodName }}</td>
...
const props = defineProps(["closeModal", "partData"]);
...
const submit = async () => {
  try {
    ...
    if (valid) {
      ...
      fd.append("partId", props.partData.partId);
...
onMounted(async () => {
  try {
    ...
    // Ambil data gambar dari server
    const res2 = await useAppStore().ajaxPost({
      url: "setup/getdrawing",
      reqData: { partId: props.partData.partId },
    });

    // Konversi Base64 menjadi File
    const designFile = base64toFile(
      res2.signFileContent, // Base64 dari respons
      `drawingPart_${props.partData.partId}.png`
    );

    // Referensi elemen file input
    const inputEl = fileInput.value.$el.querySelector("input[type='file']");

    // Gunakan DataTransfer untuk menyisipkan file
    const dataTransfer = new DataTransfer();
    dataTransfer.items.add(designFile);
    inputEl.files = dataTransfer.files;

    // Perbarui `part.designFile`
    part.designFile = designFile;

    // onFileSelected(designFile, `drawingPart_${props.partData.partId}`);

    // Update data part
    for (const key in res2.partData[0]) {
      if (key == "inspectionItem") {
        part[key] = JSON.parse(props.partData[key]);
      } else {
        part[key] = props.partData[key];
      }
    }
...
const base64toFile = (base64Data, fileName, mimeType = "image/png") => {
